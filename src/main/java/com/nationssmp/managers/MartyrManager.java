...
public class MartyrManager {
    ...
    public void buildMartyrGrave(Player dead, Player killer, Nation deadNation) {
        ...
        world.spawn(standLoc, ArmorStand.class, stand -> {
            ...
            eq.setBoots(finalArmour[0],        EquipmentSlot.FEET);
            eq.setLeggings(finalArmour[1],     EquipmentSlot.LEGS);
            eq.setChestplate(finalArmour[2],   EquipmentSlot.CHEST);
            eq.setHelmet(finalArmour[3],       EquipmentSlot.HEAD);
            eq.setItemInMainHand(sword,        EquipmentSlot.HAND);
            // Lock all equipment so it cannot be taken
    ...
    private void placeBannerOnWall(Block block, Nation nation) {
        ...
        if (state instanceof Banner banner) {
            ...
            if (bannerItem.hasItemMeta() && bannerItem.getItemMeta() instanceof BannerMeta meta) {
                ...
                if (meta.hasColor()) banner.setBaseColor(meta.getBase());
            }
