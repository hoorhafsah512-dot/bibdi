...
public class BannerManager {
    ...
    private static final PatternType[] ALL_PATTERNS = Arrays.stream(PatternType.values())
        .filter(p -> p != PatternType.BASE)
    ...
    public void openPatternMenu(Player player) {
        ...
        for (int i = start; i < Math.min(start + 36, ALL_PATTERNS.length); i++) {
            ...
            if (meta != null) {
                meta.setDisplayName(ChatColor.YELLOW + wordCase(type.getIdentifier()));
                List<String> lore = new ArrayList<>();
                lore.add(ChatColor.GRAY + "Click to add this pattern");
                lore.add(ChatColor.DARK_GRAY + "Pattern ID: " + type.getIdentifier());
                meta.setLore(lore);
    ...
    public void openPatternColorPicker(Player player) {
        ...
        inv.setItem(22, hint(Material.PAPER,
            ChatColor.YELLOW + "Colouring: " + wordCase(pending.getIdentifier()),
    ...
    public void handlePatternColorClick(Player player, int slot) {
        ...
        player.sendMessage(ChatColor.GREEN + "Layer added: " + wordCase(type.getIdentifier())
    ...
    private void finalizeBanner(Player player) {
        ...
        for (Pattern p : playerLayers)
            encoded.add(p.getPattern().getIdentifier() + ":" + p.getColor().name());
